...
const { renderCoinPng, renderBagPng } = require("../server/pushBanner");
...
for (const size of [72, 150]) {
  const buf = renderBagPng(size);
  const file = path.join(outDir, `shopee-bag-${size}.png`);
  fs.writeFileSync(file, buf);
  console.log("wrote", file, buf.length, "bytes");
}
