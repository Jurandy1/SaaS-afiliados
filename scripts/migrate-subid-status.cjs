...
async function main() {
  loadEnv();
  const sql = fs.readFileSync(path.join(__dirname, "..", "sql", "migrate-subid-status.sql"), "utf8");
  ...
  async function run(cs, label) {
    ...
    console.log(`Aplicando migrate-subid-status (${label})…`);
    await client.query(sql);
    const { rows } = await client.query(`
      select conname, pg_get_constraintdef(oid) as def
      from pg_constraint
      where conrelid = 'subid_ops'::regclass
        and conname = 'subid_ops_status_check'
    `);
    console.table(rows);
    await client.end();
  ...
  try {
    await run(pooler, "pooler");
  } catch (err) {
    console.log("Pooler falhou:", err.message);
    await run(direct, "direto");
  }
}
