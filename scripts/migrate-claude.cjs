...
async function main() {
  loadEnv();
  const sql = fs.readFileSync(path.join(__dirname, "..", "sql", "migrate-claude.sql"), "utf8");
  ...
  async function run(cs, label) {
    ...
    console.log(`Aplicando migrate-claude (${label})…`);
    await client.query(sql);
    const { rows } = await client.query(`
      select column_name, data_type, column_default
      from information_schema.columns
      where table_name = 'app_settings'
        and column_name in ('claude_api_key', 'claude_model')
