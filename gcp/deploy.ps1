...
Write-Host "Projeto: $ProjectId  região: $Region  serviço: $Service"
...
if (-not $Url) { throw "Não foi possível obter a URL do Cloud Run" }
...
Ensure-SchedulerJob "saas-afiliados-recent" "15 */2 * * *" "recent"
Ensure-SchedulerJob "saas-afiliados-daily" "0 4 * * *" "daily"

Write-Host ""
Write-Host "Pronto. Google Cloud Scheduler puxa sozinho."
Write-Host "  recent: a cada 2h (BRT)"
Write-Host "  daily:  04:00 BRT"
Write-Host "Teste: gcloud scheduler jobs run saas-afiliados-recent --location=$Region --project=$ProjectId"
